...
#endif
	a2560u_debug("ym262_reset: set YM262_REG_TEST to 0");
	ym262_write_reg(YM262_REG_TEST, 0);
	a2560u_debug("ym262_reset: set OPL3 mode");
...
#if YM262_DEBUG
		a2560u_debugnl("%d: freq:%d block:%d fnum:%d\n", i, scale[i], blocks[i], fnums[i]);
#else		
		// a2560u_debugnl("%d: freq:%lu block:%d fnum:%d\n", i, scale[i], blocks[i], fnums[i]);
#endif		
	}
